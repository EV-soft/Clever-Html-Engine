<?php   $DocFil= './Proj1/demoFile/CustomerOrder.page.php';    $DocVer='1.0.0';    $DocRev='2021-01-24';     $DocIni='evs';  $ModulNr=0; ## File informative only
...
htm_PagePrep($pageTitl='OrderCreate.page.php', $ØPageImage= $ØProgRoot.'_assets/images/_background.png',$align='center',
            $PgInfo=lang('@Example: Customer-ORDER Build with <b style="color:darkgreen;">PHP2HTML</b>'),
            $PgHint=lang('Tip: Toggle fullscreen-mode with function key: F11'));
...
//arrPrint($transTable['da']['Billing:'],"transTable['da']['Billing:'");
    ...
    htm_PanlHead($frmName='orders', $capt=lang('@Find existing order:'), $parms='', $icon='fas fa-search', $class='panelW960', $where=__FILE__, $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);');
        ...
        htm_Table(
            ...
            $RowBody= array( # ['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:placeholder','7:default','8:select'], ['Næste record'],... # Generel struktur!
                    ...
                    ['@Seller',       '7%','text', '',   ['left'  ],  '@The employer with contact to the customer','@Sell...'],
                    ['@Amount',      '10%','text', '2d', ['right' ],  '@The total order sum','@Amount...'],
                    ['@Currency',     '4%','ddwn', '',   ['center'],  '@Currency code for the currency used on the specification.','@Curr...','',[CurrencyArr(),'width: 55px;']],
                    ['@Maturity',     '4%','date', '',   ['center'],  '@Due date of the amount','@Due...'],
                    ['@Status',       '9%','ddwn', '',   ['left'  ],  '@Status','@Status...',  '', [OrdrStatu(),'width: 70px;']], //  ORD_Status()
                ),
            ...
            $TblNote= '',           # HTML-string
            /* $TblData=  */ $arrOrders,
            $fldNames=['ord_id', 'ord_ix', 'ord_odate', 'ord_ddate', 'ord_acco', 'ord_name', 'ord_sell', 'ord_total', 'ord_currency', 'ord_duedate', 'ord_stat'],
    ...
    htm_PanlHead($frmName='', $capt=lang('@Create new or modify order:'), $parms='', $icon='fas fa-pen', $class='panelW960', $where=__FILE__, $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);',$closWidth='',$panlHint='Demo ! <br>No connection to a DataBase.
...
if ($KIS!=true) {
    ...
    htm_PanlHead($frmName='content', $capt=lang('@Content of the order:'), $parms='', $icon='fas fa-pen', $class='panelW960', $where=__FILE__, $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);');
        global $ordrTotal;
        $link= '#';
        $ordrTotal= 0;
        htm_Table(
            ...
            $RowBody= array( # ['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:placeholder','7:default','8:select'], ['Næste record'],... # Generel struktur!
                ...
                ['@Item no.',     '9%','text', '',  ['center'],'@Item number for the service','Item...'],
                ['@Number',       '3%','text', '1d',['center'],'@Quantity stated as number','@Numb...'],
                ['@Unit',         '6%','text', '',  ['left'  ],'@Unit designation','@Unit...'],
                ['@Description', '26%','text', '',  ['left'  ],'@Description of the product / service','@Dest...'],
                ...
                ['@Currency',     '5%','ddwn', '',  ['center'],'@Currency code for the currency used on the specification.','@Curr...','',[CurrencyArr(),'width: 55px;']],
              //['@Status',       '9%','ddwn', '',  ['left'  ],'@Status',                                               '@Status...',  '',[OrdrStatu(),'width: 65px;']],
                /* ['@Maturity',     '9%','date', '',  ['center'],'@Due date of the amount','@Due...'], */
            ),
            $RowSuff= array( # ['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:value!'], ['Næste record'],... # Generel struktur!
                ['@Del.', '2%','text', '',  ['center'],
    ...
    htm_PanlHead($frmName='handling', $capt=lang('Handling the offer/order:'), $parms='', $icon='fas fa-check', $class='panelW960', $where=__FILE__, $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: lightgray;');
        htm_nl(1);
    ...
    // Menu_BottomScroll();
    htm_Dialog($capt='INFO', $content='This demo stores data to JSON text files. <br>No connection to a SQL-database.<br>There is also a lack of functionality.', $bgColor='lightyellow'/* , $buttons=[] */);
htm_PageFina();
