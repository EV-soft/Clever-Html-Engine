<?php   $DocFil= './Proj1/demoFile/CustomerOrder.page.php';    $DocVer='1.0.0';    $DocRev='2020-07-20';     $DocIni='evs';  $ModulNr=0; ## File informative only
	...
	for ($i= 0; $i < count($arrTmp[$firstId]); $i++) {
		foreach ($arrTmp as $key=>$val) {
			switch ($key) {
				case 'ord_total' :
				case 'total' :
				case 'price' : $val= str_replace([" ","."], ["",""], $val); break;    // Beløb skal renses for tusind-seperator !
			}
			$arrRow[$key] = $val[$i];
		}
		$result[]= $arrRow;
...
$bytes= 0;
# activated buttons:
if (isset($_POST['btn_sav_orders'  ])) { tabl2arr($arrOrders,'ord_id'); $bytes+= FileWrite_arr($dPath.$filepath= 'arrOrders.dat.json',$arrOrders );}
if (isset($_POST['btn_sav_content' ])) { tabl2arr($arrContent,'post');  $bytes+= FileWrite_arr($dPath.$filepath='arrContent.dat.json',$arrContent);}
...
htm_PagePrep($pageTitl='OrderCreate.page.php', $ØPageImage='../_background.png',$align='center',$PgInfo=lang('@Example: Customer-ORDER'),$PgHint=lang('Tip: Toggle fullscreen-mode with function key: F11'));
...
//arrPrint($transTable['da']['Billing:'],"transTable['da']['Billing:'");
	...
	htm_PanlHead($frmName='orders', $capt=lang('@Find existing order:'), $parms='', $icon='fas fa-search', $class='panelW960', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);');
		htm_Table(
			$TblCapt= array(
					['@Created', 'Width', 'html', 'OutFormat', 'horJust', 'Tip', 'placeholder', '@Customer orders']
				),
			...
			$RowBody= array( #['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:placeholder','7:default','8:select'], ['Næste record'],... # Generel struktur!
					...
					['@Status',       '9%','osta', '',   ['left'  ],  '@Status','@Status...'] //  ORD_Status()
				),
	...
	htm_PanlHead($frmName='', $capt=lang('@Create new or modify order:'), $parms='', $icon='fas fa-plus', $class='panelW960', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);');
		...
		htm_PanlHead($frmName='customr', $capt=lang('Customer:'), $parms='', $icon='fas fa-user', $class='panelW280', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: white;');
			...
			htm_Input($type='opti',$name='custopsl',$valu=$arrCustomr[$name], $labl='@Customer Lookup',     $hint='@Here you select which existing customer to select',$plho='', $width='50%',$algn='left',$unit='',$disa=false,$rows='3',$step='',$more='',$list= []); //CustList());
			...
			htm_MiniNote('@Orange boxes are required.');
		...
		htm_PanlHead($frmName='deliver', $capt=lang('Delivery:'), $parms='', $icon='fas fa-truck', $class='panelW280', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: white;');
			...
			htm_AcceptButt($labl='@Delivery note', $hint='@Show delivery note for delivery', $btnKind='search', $frmName='deliver', $width='', $akey='l', $proc=true,  $tipplc='LblTip_text', $tipstyl='',
...
if ($KIS!=true) {
		...
		htm_PanlHead($frmName='contact', $capt=lang('Person contact:'), $parms='', $icon='fas fa-phone-square', $class='panelW280', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: white;');
			function ContaktPers($arrCont,$value,$no='') {
				...
				htm_hr('lightgray');
				htm_AcceptButt($labl='@Delete', $hint='@Remove this contact person <br> (DEMO yet!)', $btnKind='erase', $frmName='contact_'.$no, $width='', $akey='', $proc=true, $tipplc='', $tipstyl='position: absolute; bottom: 8px;',$clickFunction='toast("Remove contact<br>Cant do it yet !","lightyellow","black")');
    ...
    //      foreach ($arrContact as $key => $value)  ContaktPers($arrContact,$value,$no=$i++);
			...
			htm_AcceptButt($labl='@Create new', $hint='@Create a new contact <br> (DEMO yet!)', $btnKind='create', $frmName='contact', $width='', $akey='', $proc=true, $tipplc='', $tipstyl='position: absolute; bottom: 8px;',$clickFunction='toast("Create contact<br>Cant do it yet !","lightyellow","black")');
		htm_PanlFoot($labl='@Save', $subm=true, $title='', $btnKind='save', $akey='', $simu=false, $frmName);
		...
		#      dev_show(); //  echo 'SESSIONS variablers indhold: ';  vis_data($_SESSION);
			...
			htm_hr('lightgray');
			htm_AcceptButt($labl='@Search', $hint='@Start search in the CVR register', $btnKind='create', $frmName='cvrform', $width='', $akey='s', $proc=true);
	...
	htm_PanlHead($frmName='content', $capt=lang('@Content of the order:'), $parms='', $icon='fas fa-plus', $class='panelW960', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: rgba(240, 240, 240, 0.80);');
		$link= '#';
		htm_Table(
			...
			$RowBody= array( #['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:placeholder','7:default','8:select'], ['Næste record'],... # Generel struktur!
				['@Pos.',         '5%','indx', '',  ['center'],'@Pos. No. is assigned automatically','...pos...'],
				['@Item no.',     '8%','text', '',  ['center'],'@Item number for the service','Item...'],
				['@Number',       '3%','text', '',  ['center'],'@Quantity stated as number','@Numb...'],
				['@Unit',         '6%','text', '',  ['left'  ],'@Unit designation','@Unit...'],
				['@Destription', '30%','text', '',  ['left'  ],'@Description of the product / service','@Dest...'],
				['@VAT' ,         '5%','text', '',  ['center'],'@VAT pct. rate','@vat...'],
			...
			$RowSuff= array( #['0:ColLabl', '1:ColWidth', '2:InpType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:value!'], ['Næste record'],... # Generel struktur!
				['@Undo',      '2%','text', '',  ['center'],
							  ...
							  lang('@Reverse this entry, e.g. undo reminder fee').'"></ic></a>'],
				['@Move',         '2%','text', '',  ['center'], '@Move an entry up or down.',
							  '<a href='.$link.'><ic class="fas fa-arrows-alt-v" style="font-size:14px; color:green;" title="'.
							  lang('@Not working yet').'"></ic></a>']
				),            # Felt 4: ($fieldModes), er sammensat af: [horJust, FeltBgColor, FeltStyle, SorterON, FilterON, SelectON, ]
			$TblNote=   '<small>This table contains an example of automatic calculation:<br>$total= ($DataRow[2]*$DataRow[6])*(100-$DataRow[7])/100*(100+$DataRow[5])/100;</small>',
			$data = $arrContent,
			$fldNames=['post', 'product', 'numb', 'unit', 'description', 'vat', 'price', 'discount', 'total', 'currency', 'duedate'],           #
	...
	htm_PanlHead($frmName='handling', $capt=lang('Handling the order:'), $parms='', $icon='fas fa-check', $class='panelW960', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: lightgray;');
		...
		htm_Input($type='dec2', $name='totl', $valu= 0,          $labl='<b>'.lang('@Total:').'</b>',    $hint='@System field: Amount incl. VAT', $plho= '',  $width='120px',$algn='center',$unit=' DKK ',$disa=true,$rows='1',$step='',$more='');
		htm_nl(2);
		...
		htm_AcceptButt('@Make Delivery Note',lang('@Make delivery note for the shipment of the order'),     $btnKind='create', $frmName='doNote',   $width='140px', $akey='', $proc=true, $tipplc='LblTip_text', $tipstyl='position: absolute; bottom: 50px;',$clickFunction='toast("Make delive note<br>Cant create yet !","orange","black")');
		htm_AcceptButt('@Create Invoice ',  lang('@Create invoice for (the saved!) Order'),                 $btnKind='create', $frmName='doInvo',   $width='140px', $akey='', $proc=true, $tipplc='LblTip_text', $tipstyl='position: absolute; bottom: 50px;',$clickFunction='toast("Create invoice<br>Cant create yet !","orange","black")');
		...
		htm_AcceptButt('@Delete',           lang('@Delete the order - provided the invoice is not formed'), $btnKind='erase',  $frmName='doErase',  $width='140px', $akey='', $proc=true, $tipplc='LblTip_text', $tipstyl='position: absolute; bottom: 50px;',$clickFunction='toast("Delete<br>Cant erase yet !","orange","black")');
		//  $proc=false, $tipplc='LblTip_text', $tipstyl='',$clickFunction='', $more )
		htm_nl(2);
	htm_PanlFoot();

		htm_nl(1);

	htm_PanlHead($frmName='language', $capt='Settings:', $parms='', $icon='fas fa-wrench', $class='panelW320', $func='Undefined', $more='', $BookMark='blindAlley.page.php');
	...
	htm_PanlHead($frmName='', $capt='Info about this page:', $parms='', $icon='fas fa-info', $class='panelW320', $func='Undefined', $more='', $BookMark='blindAlley.page.php',$panlBg='background-color: lightyellow; ');
