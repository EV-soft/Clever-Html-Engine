<?   $DocFile='../Proj1/php2html.lib.php';    $DocVers='1.0.0';    $DocRev1='2020-07-20';     $DocIni='evs';  $ModulNo=0; ## File informative only
...
function arrPrint($arr,$name='',$proc=true) {  ## Output actual value of any variabeltype
	if ($name>'') $name.=': ';
	$result= "<pre>".$name. print_r($arr). "</pre><hr>\n";
	if ($proc) echo $result;
	else return $result;
...
function htm_Input(# $type='',$name='',$valu='',$labl='',$hint='',$plho='@Enter...',$wdth='',$algn='left',$unit='',$disa=false,$rows='2',$step='',$more='',$list=[],$llgn='R',$bord='');
...
#INPUT:
	...
	$inpStyle= ' class="boxStyle" style="text-align: '.$algn.'; font-size: 14px; font-weight: normal; '.$bord; //boxStyle - border: 1px solid var(--grayColor);
...
function htm_Caption($labl='',$style='color:#550000; font-weight:600; font-size: 13px;',$align='') {
  echo '<data-colrlabl style="'.$style.$align.'">'.lang($labl).'</data-colrlabl>';
}
function htm_TextDiv($content,$align='left',$marg='8px',$more='') {
...
|-------------------------------------------------------------------------------------------------------|
*/
function htm_Table(# $TblCapt,$RowPref,$RowBody,$RowSuff,$TblNote,&$TblData,$FilterOn,$SorterOn,$CreateRec,$ModifyRec,$ViewHeight,$TblStyle,$CalledFrom$Criterion)
	$TblCapt= array( #['0:Label',   '1:Width',    '2:Type',    '3:OutFormat', '4:horJust',      '5:Tip',    '6:placeholder', '7:Content';], ['Næste record']
		),
	$RowPref= array( #['0:ColLabl', '1:ColWidth', '2:ContType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip'                                        ], ['Næste record'],... # Generel struktur!
		),           // if (($ModifyRec) or ($RowBody[0][2]!='indx')) er 2% ColWidth benyttet til => knap
	$RowBody= array( #['0:ColLabl', '1:ColWidth', '2:ContType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:placeholder','7:default','8:select'], ['Næste record'],... # Generel struktur!
		),
	$RowSuff= array( #['0:ColLabl', '1:ColWidth', '2:ContType', '3:OutFormat', '4:[horJust_mv]', '5:ColTip', '6:value!     '                       ], ['Næste record'],... # Generel struktur!
		),            # Felt 4: ($fieldModes), is composed of: [horJust, FeltBgColor, FeltStyle, SorterON, FilterON, SelectON, ]
	$TblNote= '',           # HTML-string
	&$TblData,              # = array(),
	$fldNames=[],           # FieldNames in array returned on submit. Also used to sort data fields
...
### TableFooter with the options to create a new record:
	...
	if ($CreateRec) { ## Create new data in a new tabelRow:
		echo '  <tr>';  //  Footer-Row with createbutton:
		if (($selectable) or (count($RowPref)>=1)) echo  '<td> </td>';
		if (count($RowPref)>=2) {$colsp= 'colspan="2"'; $n= 2; } else {$colsp= ''; $n= 1; }
		echo '  <td style="font-size: 12px;" '.$colsp.'>'.lang('@Create new:').'</td>';
		for ($x= $n; $x < count($RowPref)+count($RowBody)-$hiddcount-1; $x++)
			echo '<td> </td>';
		echo '<td style="text-align:center;">'.
			 htm_AcceptButt($labl='@Create record',
							...
							$form='form_'.$ØPanelIx.'_'.$ØTblIx, $width='', $akey='c', $proc=false, $tipplc='LblTip_NW', $tipstyl='position: absolute; bottom: 80px; right: 100px;',$clickFunction='toast("Create record<br>Cant create yet !","orange","black")').
			 '</td>';
		for ($x= 0; $x <= count($RowSuff); $x++)
			echo '<td style="width:'.$RowPref[1].'; display: none;"> </td>';
		echo ' </tr>';

		echo '  <tr>';  #  Footer-Row with input-fields:
		if ($selectable) echo '<td style="width:0.5%;"> </td>';
		...
		foreach ($RowBody as $Body) { $ColIx++;
			...
			if ($Body[6]=='@Numb...') $oblg= lang('@Mandatory'); else $oblg= '';
			if (($GLOBALS["Øshow"]>0) and ($Body[2]=='hidd')) $Body[2]= 'text';
			switch ($Body[2]) {  # Special ContTypes ("Lookup"):
			...
			# case 'font' : echo '<td'.$s1.ddwnList($s2,$valu,FontListe(),$bgclr.'width:75px; ').'</td>';  break;
				...
				case 'hidd' : echo '<td style="width:0; padding:0; display:none; '.$bord.'">  <input type= "hidden" name="'.$fldNames[$ColIx].'[]" '.
		...
		$ColIx= -1; foreach ($RowSuff as $Suff) {$ColIx++; if ($ColIx>=0) echo '<td style="display: none"></td>';}
		//echo '<td>'.'</td>';
		echo ' </tr>';
	}
	echo '  </tfoot>';
...
### DATA and html-objects:
	...
	if ($TblData)
		foreach ($TblData as $Drow) {
			...
			if ($selectable) echo '<td style="text-align:right; width:2%;">'.$RowSelect.'</td>';
			...
			## Specielle betingede "række"-formater:
				...
				### SPECIAL:
					case 'calc' : /* if ($Body[8][3] == '2d') */ {
									// [1, '45-876', 2:$antal=3, 'stk', 'Redekasser', 5:$momssats=25, 6:$pris=235.50, 7:$rabat=8,  $ialt=($antal*$pris)*(100-$rabat)/100*(100+$momssats)/100, 'DKK'],
										$ialt= ($Drow[2]*$Drow[6])*(100-$Drow[7])/100*(100+$Drow[5])/100;
								  echo '"> <input type= "text" name="'.$fldNames[$ColIx].'[]" '.
									   'value="'.number_format((float)$ialt, 2,',',' '). '" placeholder="'.lang($Body[6]).'"'.
				...
				### STANDARD:
					case 'date' : if (($valu==' ') /* or ($valu==NULL) */) $clr= 'color: transparent; '; else $clr= '';  // Skjul browserens placeholder ved at angive SPACE
								  echo '">'.'<input type= "date" name="'.$fldNames[$ColIx].'[]" '. //  (id="'.$name.'")
										  'style="line-height:100%; text-align:left; font-size:normal; height:16px; max-width: 150px; '.$clr. $inpBg.
					...
					case 'show' : if ($valu==' ') $clr= 'color: transparent; ';                                   //  Kun visning af data:
								  ...
								  break;
					case 'helt' : echo '"> <input type= "text" name="'.$fldNames[$ColIx].'[]" '.
					...
					case 'keyn' : //  Selectable and editable index
								  echo '"><span style="font-size:small"  name="'.$fldNames[$ColIx].'[]" title="'.lang('@The row is selectable. Click here to edit the row`s fields').'">'.RowKlick($ModifyRec,$valu,$RowIx,$ColIx,$fldNames,$CalledFrom).'</span>';
								  break;
					...
					case 'blnk' : //  Value is displayed as BLANK
								  echo '"><span name="'.$fldNames[$ColIx].'[]"  > </span>';
								  break;
					case 'hidd' : //  Hidden data is included as hidden columns to have a complete record (simplifies updating):
								  echo 'width:0; padding:0; border:none; display:none;">  <input type= "hidden" name="'.$fldNames[$ColIx].'[]" '.  //   visibility:hidden;
									   'value="'.htmlentities(stripslashes(lang($valu))).'" '.$txAlign.$inpBg.' width:0;" /> ';
								  break;
								 // text,
					default   : { echo '"> <input type= "text" name="'.$fldNames[$ColIx].'[]" value="'.$valu.'" '.
...
{ # MUST be followed of htm_PanlFoot !
	...
	$togg= '<span style="color: black; font-size: 16px; display:inline-block; white-space: nowrap; width:12px; height:12px;
			margin-top:6px; margin-right:4px; float:right; font-size: smaller;"><i class="fas fa-exchange-alt  fa-rotate-90"></i></span>';
...
## PANEL-START:
	echo '<span class="'.$class.'" id="panel'.$ØPanelIx.'" '.$more.' style="margin: 0 10px 10px 0; position: relative; left: -6px; vertical-align: top; '.$panlBg.'"> '. $formCrea.
			'<span style="display: inline-block; width:100%; text-align: left;">'.
				...
				'data-tiptxt="'. lang('@Click icon or panel header-text to open / close this panel').'" title="'. lang('@Click icon or panel header-text to open / close this panel').
				'" onclick= PanelSwitch'.$ØPanelIx.'(); >';  //  Panel-Header
	echo  '<ic class="'.$icon.'" style="font-size: 20px; color: '.$ØiconColor.';"> </ic> &nbsp;'.ucfirst(lang($capt)).$fn; //.$togg;
	echo $togg;
	echo       '</span>'; // panelTitl

	echo  '<ic class="fas fa-angle-double-up" style="width:12px; height:12px; margin-top:6px; margin-right:4px; float:right; cursor:zoom-out; font-size: smaller;" '.
			'title="'. lang('@Click to close all panels').';" onclick= PanelMinimizeAll(); ></ic>';
	echo  '<ic class="fas fa-angle-double-down" style="width:12px; height:12px; margin-top:6px; margin-right:0px; float:right; cursor:zoom-in; font-size: smaller;" '.
...
### COLUMNS:
function RowColTest($colr) {if (DEBUG) return ' style="border: 3px solid '.$colr.';"'; else return '';}
// Columns, layout in windows. Is used in xxx.page.php-files:
function htm_RowColTop ($RowColWdth=240) {dvl_pretty('htm_RowColTop');
								 echo '<data-ColnHead'.RowColTest('yellow').'> <span id="colnwrap" '.RowColTest('green').'> '.
								  '<data-RowCol id="RowCol'.$RowColWdth.'" '.RowColTest('blue').' >'; } // RowColTop RowCol240, RowCol320 (Look at CSS ! )
function htm_RowColNext($RowColWdth=320) {
								 echo '</data-RowCol> <data-RowCol id="RowCol'.$RowColWdth.'" '.RowColTest('red').'>'; }
function htm_RowColBott()       {echo '</data-RowCol> </span></data-ColnHead><span class="clearWrap" >'; }
## $RowColWdth - Only use defined width !



function htm_AcceptButt( # $labl='', $title='', $btnKind='', $form='', $width='', $akey='', $proc=false, $tipplc='LblTip_text', $tipstyl='',$clickFunction='', $more);
...
## Action:
	if ($clickFunction=='') {$type='submit';} else {$type='button';}
## Function:
	...
	$result.= '  <button class="acceptbutt" '.$form.' type= "'.$type.'" name="btn_'.$midn.$name.'" id="btn_'.$midn.$name.'" '.$more.
			  '  style="'.$width. $colors.' min-height: 28px;" onclick=\''.$clickFunction.'\' accesskey="'.$akey.'"> '. ucfirst(lang($labl)).
			  '  </button>';
...
function htm_PagePrep($pageTitl='', $ØPageImage='',$align='center',$PgInfo='',$PgHint='') { # Prepare / initialize a page
...
### ----------------------Library-jQuery-------------------------
### jQuery-latest:
	echo '<script src="'.$ØProgRoot.'_assets/jquery/3/jquery-3.3.1.js"></script>'; // latest //  emne="Tablesorter-system" og Topmenu-system
...
### ----------------------Library-fontawesome icons ----------------------
	...
	if ($PgInfo>'')
		echo '<div style="position: fixed; width: min-content; bottom: 10px; right: -'.(9.3 * strlen($PgInfo)).'px; z-index: 99;
			transform-origin: bottom left; transform: rotate(-90deg); white-space: nowrap; padding: 3px;">'.$PgInfo.'</div>';
	...
	if ($PgHint>'')
		echo '<div style="position: fixed; width: min-content; bottom: -10px; left: 0px; z-index: 99;
			transform-origin: top left; transform: rotate(-90deg); white-space: nowrap; padding: 3px;">'.$PgHint.'</div>';
	if (is_null($panelCount)) $panelCount= 20;
	PanelInit();
...
function htm_PageFina() { global $ØPanelIx, $panelCount, $ØProgRoot;
	$panelCount= $ØPanelIx;
